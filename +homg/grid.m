classdef grid < handle
  ...
  methods
    ...
    function u = vcycle(grid, v1, v2, rhs, u)
	  ...
	  if grid.prec_switch==3 || grid.prec_switch==5 || grid.prec_switch==6 || grid.prec_switch==7
	     u = double(single(u) - single(grid.P * u_corr_coarse)) ; % -- the multiplication with sparse matrix requires double precision
	  else
	...
	function set_prec_switch(grid, ps)
	   grid.prec_switch = ps ;
	   if( ~ isempty(grid.Coarse))
	      grid.Coarse.set_prec_switch(ps);
	   end
	end
    ...
    function u = smoother_jacobi (grid, v, rhs, u)
      ...
      for i=1:v
	    if grid.prec_switch==1 || grid.prec_switch==4 || grid.prec_switch==6 || grid.prec_switch==7
		    res = single(grid.jacobi_invdiag) .* single(grid.residual(rhs, u)) ;
			  u = double(u - grid.jacobi_omega.*res) ;
		else
    ...
    function u = smoother_sym_sor (grid, v, rhs, u)
      ...
      for i=1:v
        r = grid.residual(rhs, u);
		if grid.prec_switch==1 || grid.prec_switch==4 || grid.prec_switch==6 || grid.prec_switch==7
		   u = double( u - single(grid.ssor_M \ r));
		else
		   u = u - grid.ssor_M \ r;
		end
        u = grid.ssor_M' \ (grid.ssor_N'*u + rhs); % -- Try this statement later: supposedly doesn't supports SP with sparse matrix
      end
    end
    ...
    function set_sor_omega(grid, w)
      ...
      grid.ssor_N = [];
        ...
        Kc = spdiags(grid.jacobi_invdiag,0,length(D), length(D)) * grid.K;
			...
			if grid.prec_switch==1 || grid.prec_switch==4 || grid.prec_switch==6 || grid.prec_switch==7
			    beta = single(grid.eig_max); %--making this sp progressively make future computations sp
			else
			    beta = grid.eig_max;
			end
